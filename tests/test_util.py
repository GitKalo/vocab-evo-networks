...
class TestUtil(unittest.TestCase) :
    def test_pick_item_success(self) :
        ...
        item = src.util.pick_item(list_prob)
    ...
    def test_pick_item_wrong_list_distribution(self) :
        list_items = np.random.randint(1, 10, size=10)
        self.assertRaises(AssertionError, src.util.pick_item, list_items)
    
    def test_pick_item_wrong_list_values(self) :
        self.assertRaises(ValueError, src.util.pick_item, [])
        self.assertRaises(ValueError, src.util.pick_item, ['test'])
        self.assertRaises(ValueError, src.util.pick_item, [1, 'test', 0.2])
    ...
    def test_pick_item_wrong_arg_type(self) :
        self.assertRaises(TypeError, src.util.pick_item, 5)
        self.assertRaises(TypeError, src.util.pick_item, 'test')
